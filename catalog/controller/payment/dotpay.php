...
class ControllerPaymentDotpay extends Controller {
    ...
    private function isValid($params, $order){
         
        if ($params['operation_original_currency'] !== $order['currency_code']) {            
            $this->error['error_original_currency'] = 'error_original_currency';          
        }
            
        if ($params['operation_original_amount'] !== number_format($this->currency->format($order['total'],$order['currency_code'], $order['currency_value'], FALSE), 2, '.', '')) {
            $this->error['error_amount'] = 'error_amount';            
        }
