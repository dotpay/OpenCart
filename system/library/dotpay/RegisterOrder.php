...
class RegisterOrder {
    ...
    private function createRequest($data) {
        try {
            $curl = new Curl();
            $curl->addOption(CURLOPT_URL, $this->config->get(self::PLUGIN_NAME.'_target_payment_url').self::target)
                 ...
                 ->addOption(CURLOPT_USERPWD, $this->config->get(self::PLUGIN_NAME.'_username').':'.$this->config->get(self::PLUGIN_NAME.'_password'))
    ...
    private function checkIfCompletedControlExist($control, $channel) {
        ...
        $payments = $this->SellerApi->getPaymentByOrderId($this->config->get(self::PLUGIN_NAME.'_username'), $this->config->get(self::PLUGIN_NAME.'_password'), $control);
        foreach($payments as $payment) {
            $onePayment = $this->SellerApi->getPaymentByNumber($this->config->get(self::PLUGIN_NAME.'_username'), $this->config->get(self::PLUGIN_NAME.'_password'), $payment->number);
