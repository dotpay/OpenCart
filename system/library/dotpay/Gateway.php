...
class Gateway {
    ...
    public function getHiddenFields() {
        $this->load->language($this->getName());
        $data = array();
    ...
    public function confirm() {
        ...
        $this->load->model(dirname(dirname($this->getName())).'/dotpay_new');
        ...
        if($this->getDataFromRequest('operation_type') == self::OPERATION_TYPE_PAYMENT) {
            ...
            if($newOrderState == $this->config->get(self::PLUGIN_NAME.'_status_completed')) {
                $cc = $this->model_payment_dotpay_oc->getCreditCardByOrder($this->order['order_id']);
                if($cc !== NULL && $cc['cc_id'] !== NULL && $cc['card_id'] == NULL) {
                    ...
                    $this->model_payment_dotpay_oc->updateCard($cc['cc_id'], $ccInfo->id, $ccInfo->masked_number, $ccInfo->brand->name); 
                }
            }
            $this->load->model('checkout/order');
            ...
            $return = $this->model_payment_dotpay_new->getReturnByOrderId($this->order['order_id']);
            if (!$return) {
    ...
    private function returnOperation($return) {
        if (!empty($return) && $return['return_status_id'] != $this->config->get(self::PLUGIN_NAME.'_status_return') ) {     
            if ($this->request->post['operation_status'] == self::OPERATION_STATUS_COMPLETED) {          
                ...
                $this->model_payment_dotpay_new->addReturnHistory($return['return_id'], $data);
            }            
        }
    }
    ...
    private function getHiddenFieldsOc() {
        ...
        if($this->request->post['oc_type'] == 'new') {
            ...
            $data['credit_card_customer_id'] = $this->model_payment_dotpay_oc->addCard($this->session->data['customer_id'], $this->order['order_id']);
        } else {
            $card = $this->model_payment_dotpay_oc->getCardById($this->request->post['card_id']);
            if($card == NULL)
                return $data;
