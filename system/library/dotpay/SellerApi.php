...
class SellerApi {
    ...
    public function isAccountRight($username, $password, $version) {
        ...
        if(empty($username) && empty($password))
            return true;
        $url = $this->config->get(self::PLUGIN_NAME.'_target_seller_url').$this->getDotPaymentApi()."payments/";
        $curl = new Curl();
    ...
    public function getPaymentByNumber($username, $password, $number) {
        $url = $this->config->get(self::PLUGIN_NAME.'_target_seller_url').$this->getDotPaymentApi()."payments/$number/";
        $curl = new Curl();
        $curl->addOption(CURLOPT_URL, $url)
             ->addOption(CURLOPT_USERPWD, $username.':'.$password);
        $this->setCurlOption($curl);
    ...
    public function getPaymentByOrderId($username, $password, $orderId) {
        $url = $this->config->get(self::PLUGIN_NAME.'_target_seller_url').$this->getDotPaymentApi().'payments/?control='.$orderId;
        $curl = new Curl();
        $curl->addOption(CURLOPT_URL, $url)
             ->addOption(CURLOPT_USERPWD, $username.':'.$password);
        $this->setCurlOption($curl);
