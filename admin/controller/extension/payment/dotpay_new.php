...
class ControllerExtensionPaymentDotpayNew extends Controller {
    ...
    public function index() {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validate()) {
            ...
            $this->response->redirect($this->url->link($this->getExtensionName(), 'token=' . $this->session->data['token'], 'SSL'));
        }
	...
	$data['datatable_language'] = (strtolower(substr(trim($this->language->get('code')),0,2))=='pl')?'Polish':'English';
        ...
        $data['ocmanage_remove_url'] = HTTPS_SERVER . 'index.php?route='.$this->getExtensionName().'/ocremove&token='.$this->session->data['token'];
        ...
        $this->response->setOutput($this->load->view($this->getExtensionName().'.tpl', $data));
    }
    ...
    private function getBreadcrumbs() {
        ...
        $breadcrumbs[] = array(
            'href' => HTTPS_SERVER . 'index.php?route='.$this->getExtensionName().'&token=' . $this->session->data['token'],
